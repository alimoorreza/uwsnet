...
from .ifsl_base.feature import extract_feat_vgg, extract_feat_res
from .ifsl_base.correlation import Correlation
from utils import asnet_utils as utils
from utils.asnet_evaluator import Evaluator

from .ifsl_base.learner import AttentionLearner
...
class AttentiveSqueezeNetwork(pl.LightningModule):
    ...
    def predict_mask_nshot(self, query_img, support_img, support_mask):

        # Perform multiple prediction given (nshot) number of different support sets
        logit_mask_agg = 0
        nshot = support_img.shape[1]
        for s_idx in range(nshot):
            logit_mask = self(query_img, support_img[:, s_idx], support_mask[:, s_idx])
