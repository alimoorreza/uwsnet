...
from networks.prototype_mixture_models.backbone import resnet_dialated as resnet
from networks.prototype_mixture_models import ASPP
from networks.prototype_mixture_models.PMMs import PMMs
...
class OneModel(nn.Module):
    ...
    def forward_1_shot(self, query_rgb, support_rgb, support_mask):
        ...
        # feature concate
        Pseudo_mask = (torch.zeros(self.batch_size, 2, 50, 50)).cuda(self.args.GPU)
        out_list = []
        for num in self.num_pro_list:
            self.num_pro = num
            self.PMMs = PMMs(256, num, args=self.args).cuda(self.args.GPU)
    ...
    def forward(self, query_rgb, support_rgb_batch, support_mask_batch):
        # extract query feature
        query_feature = self.extract_feature_res(query_rgb)
